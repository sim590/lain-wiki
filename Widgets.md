...
Every lain widget contains a `wibox.widget.textbox`, which is updated by a timed function. To access the textbox, use the field `widget`, while to access the timed function, use the field `update`. Some lain widgets may also have an `icon` field, which is a `wibox.widget.imagebox`, and/or a `timer` field, which is the `gears.timer` on `update`.
...
To markup `widget`, call `widget:set_markup(...)` within `settings`.
